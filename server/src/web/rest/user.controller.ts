...
export class UserController {
  ...
  async getUser(@Param('login') loginValue: string): Promise<User> {
    const user: User = await this.userService.find({ where: { login: loginValue } });
    if(!user){
      throw new HttpException('There is no user with this email', HttpStatus.NOT_FOUND);
    }
    return user;
  }
